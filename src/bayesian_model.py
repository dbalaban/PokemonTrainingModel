...
def update_ev_pmf(prior: EV_PMF, upd: EV_PMF, mode: str = "linear") -> EV_PMF:
    ...
    # Fold overflow probability mass into the final bin instead of dropping it
    full = np.convolve(prior.T, upd.T)
    max_ev = prior.max_total_ev
    new_T = full[:max_ev + 1]
    overflow = full[max_ev + 1:].sum()
    new_T[-1] += overflow
    s = new_T.sum()
    if s > 0:
        new_T = new_T / s
...
def hybrid_ev_iv_update(
    ...
    *,
    ...
    for _ in tqdm(range(max_iters), desc="Hybrid EV/IV update", disable=not verbose):
        ...
        alloc = W6 * T_idx[None, :]
        EV_mat = EV_PMF._round_allocations_to_totals(alloc, T_idx, ev_post.max_ev)  # (6, M) ints
...
def analytic_update_with_observation(
    ...
    *,
    ...
    """
    # ----- constants / helpers -----
    obs = statblock_to_array(obs_stats)  # (6,)
    Bvec = statblock_to_array(base_stats)
...
def update_with_observation(
    ...
    """
    obs = statblock_to_array(obs_stats)
